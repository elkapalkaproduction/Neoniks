...
+ (BOOL)createConnectionFromRequest:(NSURLRequest *)request forDelegate:(id <PHConnectionManagerDelegate> )delegate withContext:(id)context {
    ...
    if (![NSURLConnection canHandleRequest:request]) return NO;
...
+ (void)stopConnectionsForDelegate:(id <PHConnectionManagerDelegate> )delegate {
    ...
    for (NSURLConnection *connection in[(NSMutableDictionary *)connectionManager.connections allKeys]) {
        ...
        if ([connectionBundle delegate] == delegate) {
            ...
            if ([delegate respondsToSelector:@selector(connectionWasStoppedForRequest:context:)]) [delegate connectionWasStoppedForRequest:[connectionBundle request] context:[connectionBundle context]];
...
- (void)connectionDidFinishLoading:(NSURLConnection *)connection {
    ...
    if ([delegate respondsToSelector:@selector(connectionDidFinishLoadingWithRequest:response:data:context:)]) [delegate connectionDidFinishLoadingWithRequest:request response:response data:data context:context];
...
- (void)connection:(NSURLConnection *)connection didFailWithError:(NSError *)error {
    ...
    if ([delegate respondsToSelector:@selector(connectionDidFailWithError:request:context:)]) [delegate connectionDidFailWithError:error request:request context:context];
